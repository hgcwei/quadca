#!/usr/bin/env python3
"""
QuadCA Benchmark Runner — 30-Factor Experiment
================================================
Runs QuadCA on all 100 configurations from the 30-factor benchmark suite
...
Usage:
  python run_quadca_timing.py [--configs data/benchmark_30factor_configs.csv]
                              [--exe ./QuadCA]
                              [--output quadca_results_30f.csv]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run QuadCA on 30-factor benchmark configs.")
    ...
    parser.add_argument("--exe", default=None,
                        help="Path to QuadCA executable (default: auto-detect QuadCA or QuadCA.exe)")
    parser.add_argument("--output", default="quadca_results_30f.csv",
    ...
    for i, levels in enumerate(configs):
        ...
        # QuadCA outputs "count,time" on stdout
    ...
    print(f"\n{'='*50}")
    print(f"QuadCA 30-Factor Benchmark Complete!")
    print(f"{'='*50}")
